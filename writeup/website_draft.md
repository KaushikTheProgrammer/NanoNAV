...
*The CEM planning loop. Each iteration draws a batch of candidate action plans from a Gaussian, rolls each through the world model, and scores the imagined endpoint by its latent distance to the goal. The best fraction, the elites, are kept and the Gaussian is refit around them, so the search narrows toward the plans that land closest to the goal. After a few iterations the planner executes the first chunk of the winner and replans from the new observation.*
...
*Note here the intial jerkiness in the trajectory, this is due to non-smooth waypoints generated by the planner. The footage is compressed roughly 88× from the original 12.8-minute run.*
