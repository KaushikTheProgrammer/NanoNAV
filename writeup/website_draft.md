...
*Additive injection (left) lets the model shrink the action's weight toward zero and ignore it. AdaLN (right) makes the action scale every layer, so it cannot be tuned out.*
