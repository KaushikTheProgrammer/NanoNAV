...
- **6e — semantic WM retrain (Option C) — ✅ PROTOTYPE ON-ROBOT VALIDATED (2026-06-11): 3/3 arrivals, incl. the goal flat-L2 failed. ✅ C3 SUBGOAL GRAPH BUILT + OFFLINE-VALIDATED (2026-06-11): DIRECTED + direction-certified welds (94.5% SCC), wormhole audit clean, 4/4 offline routes, `lekiwi_mpc --graph` with indexed-waypoint viewer + route strip. ⏳ FIRST ON-ROBOT A/B INCOMPLETE: baseline = plateau-wander confirmed; graph run 52→47 hops then single-node stall (diagnose .rrd steps 85–92; try --graph-lookahead 0.3) — resume. Remaining C1 items = fresh-goal floor test (reach-thresh 0.08 vs 0.2–0.25), cost-mode-first arm, optional SD-VAE baseline legs → then C2 recollect+retrain (feeds a denser graph directly).** Retrain the WM over frozen
